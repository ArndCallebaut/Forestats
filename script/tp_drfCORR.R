...
## the honesty argument splits the OOB samples in two subsamples
## one is to evaluate the splits and the other to populate the leaves
## it provides a more honest OOB evaluation of the tree, but for small
## sample sizes (<1000) it limits the number of data the model will learn from
...
### response : by default the number of OOB samples is 0.5, the total of
### the number of observations in the leaves correspond to half of the sample size

## plot the 105th tree
tree105 <- get_tree(drf_basic, 105)
...
dr_obs62 <- y_all[sample(1:length(y_all), size = length(y_all),
                        prob = w[62,], replace = TRUE)]

## get model performance
pred_ozone <- predict(drf_basic, functional = "mean")$mean[,1]
nrmse <- caret::RMSE(pred_ozone, y_all) / mean(y_all)
r2 <- caret::R2(pred_ozone, y_all)
...
## tweak model fitting
## fill in the dots
drf_adv <- drf(X_all, y_all, num.trees = 500,num.features = 25,
               sample.fraction = 0.8, mtry = 10, ci.group.size = 1,
               honesty = FALSE,compute.variable.importance = TRUE)

## compare to the basic model
pred_adv <- predict(drf_adv, functional = "mean")$mean[,1]
nrmse_adv <- caret::RMSE(pred_adv, y_all) / mean(y_all)
r2_adv <- caret::R2(pred_adv, y_all)
...
roc_urch <- roc(Y[,"urchin"], pred_lau[,2])
thresh_urch <- coords(roc_urch, "best", ret = "threshold")
caret::confusionMatrix(factor(as.numeric(pred_lau[,2] >= thresh_urch[1,1])),
                       factor(st_lau$urchin))
...
explainer <- explain(
  ...
  data = X,
  y = st_lau$urchin,
  predict_function = function(model, newdata) predict(model, newdata, functional="mean")$mean[,2],
  label = "DRF"
)

pdp_temp <- model_profile(explainer, variables = "temperature")
## add CIs - a bit clunky ... 
pdp_temp$cp_profiles$lci <- predict(m_lau,
                                    newdata = pdp_temp$cp_profiles[,names(X)],
                                    functional = "quantile", quantiles = 0.025)$quantile
pdp_temp$cp_profiles$uci <- predict(m_lau,
                                    newdata = pdp_temp$cp_profiles[,names(X)],
                                    functional = "quantile", quantiles = 0.975)$quantile
pdp_temp$agr_profiles$lci <- aggregate(lci ~ temperature, data = pdp_temp$cp_profiles, FUN = mean)$lci
pdp_temp$agr_profiles$uci <- aggregate(uci ~ temperature, data = pdp_temp$cp_profiles, FUN = mean)$uci



## plot this together with observed data
ggplot(st_lau) +
  geom_point(aes(x=temperature, y=urchin)) +
  geom_path(data=pdp_temp$agr_profiles, aes(x=`_x_`, y=`_yhat_`),
            color = "red", linewidth = 1.75) +
  geom_path(data=pdp_temp$agr_profiles, aes(x=`_x_`, y=lci),
            color = "red", linewidth = 1.25, linetype = "dashed") +
